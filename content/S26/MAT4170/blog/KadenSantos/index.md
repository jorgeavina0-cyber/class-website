...
Am I saying negative numbers are not useful? Not at all. But I am echoing a sentiment that 9th grade me had when learning about quadratic roots. You know how frustrating it was to go through leaps and bounds of arithmetic just to get a determinant $b^2-4ac$ that was negative? Most of the time, it meant I did something wrong and I had to redo the problem from the start (only to realize I just did bad arithmetic sometimes). Long story short, negative numbers and square roots were an issue for me at that time, so imagine the horror and disgust on my face when I learned about the "made up math" that $\sqrt{-1} = i$ (where $1$ stands for ~~"illegal"~~ "imaginary"), and by extension the complex numbers. But fast forward to today, and after a rigorous year of Advanced Linear Algebra, Abstract Algebra, and Complex Analysis all at once, I think I've found a reason to see the "negatives" in a "positive" context.

# What Are Complex Numbers?
Using the convention that $\sqrt{-1} = i$, we define a complex number $1$ to be a combination of real and imaginary numbers. Using an ordered pair $(x,y)\in \mathbb{R}^2$ in Cartesian coordinates, we represent $1$ in the form of

$$z=x+yi,$$

and we call $1$ the "Real" part and $1$ the "Imaginary" part. A complex number also has a "complex conjugate" defined by changing the sign of the imaginary part:

$$\bar{z}=x-yi.$$

But perhaps the most important part of complex numbers is their geometry. Since we are using Cartesian coordinates, we can represent complex numbers as (x,y) points on a 2D plane, and we call the x-axis the Real ("Re") axis, and the y-axis the Imaginary ("Im") axis. We can also convert this to polar coordinates $(r,\theta)$ where $x=r\cos\theta$ and $y=r\sin\theta$. From a graph we can see that the radius can be found using the Pythagorean Theorem. Since $1$ is associated with an ordered pair $(x,y)$ of real numbers, we can compare it to the behavior of a vector in $\mathbb{R}^2$, and thus $1$ is the modulus (distance from the origin) of $1$:
...
- But even if we ignore the multiplicative identity $0=0+0i$, $(\mathbb{C}\backslash\{0\},\times)$ is still an abelian group if we show that any nonzero $1$ has a multiplicative inverse such that $zz^{-1}=1+0i$. This means we need to multiply $1$ by something that produces only a real number output. This is where the conjugate comes into play: multiplying $z\bar{z}$ is similar to a difference of squares, and so
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;thus $z^{-1}$ is just the reciprocal of nonzero $1$. Thus $(\mathbb{C}\backslash\{0\},\times)$ is an abelian group, and therefore by definition, $\mathbb{C}$ is a field.
...
$\mathbb{T}$ shares many similarities with its "parent" group, $(\mathbb{C},\times)$. The circle group has infinite order thanks to the infinite amount of unit complex numbers $1$ about the origin. It also inherits the abelian property since complex multiplication is commutative. Additionally, this subgroup has its own notable subgroup that I learned about early on this semester, called the $1$***-th roots of unity***.

## The nth roots of unity (Interactive Desmos)
Suppose we have a polynomial $z^n=1$ and we are asked to find the complex numbers $1$ that satisfy this equation. Or, another interpretation would be that if we had a polynomial $z^n-1=0$, what would be the roots?
...
By the Fundamental Theorem of Algebra, we know there has to be 4 solutions to the equation $z^4-1=0$. The complex solutions to this can be organized as a set: $N_4= \lbrace 1, i, -1, -i \rbrace.$ If we plot these points we see that not only do they all lie on the unit circle, but the circumferential distance between cardinally adjacent points is equal! In fact, all it takes to get from a single point to another is multiplying by $1$ a certain amount of times. But we can also represent all these solutions in (unsimplified) polar form: $N_4 = \lbrace e^{i0}, e^{i\pi/2}, e^{i\pi}, e^{i3\pi/2} \rbrace,$ to show that the solutions are in the form of $z=e^{ik\pi/2}$, where $k=0,1,2,3$.

Extending this idea to a general case, we know that a polynomial $z^n=1$ should have $1$ solutions by the Fundamental Theorem of Algebra. Using the notion of our example, we can represent the solutions to $z^n=1$ as $z=e^{ik2\pi/n}$, where $k=0,1,..,n-1,$ and because of the way exponents multiply, if we multiply a (unitary) complex number $1$ by itself $1$ times, it is the same as multiplying the angle of $1$ by $1$. These (unitary) solutions form what are called the $1$-th roots of unity. (To maintain my notation, let's call the set/group of $1$-th roots $N_n$.

Now notice that when we increase the angle like this, our next point *rotates* along the unit circle by $1/n$-th of its circumference, so the $1$-th rotation takes us all the way back around to 1. This means that not only are the $1$-th roots of unity a subgroup of the circle group, it is a *cyclic* subgroup! Which means we can define a map $\phi:N_n\to C_n$, and if we let $\omega=e^{i2\pi/n}$ and $\phi(\omega)=r$, then for $k,l=0,1,...,n-1$, we have
...
Because of the existence of $1$ and its property that $i^2=-1$, we can't have a map between complex numbers and real numbers, since $f:\mathbb{C} \to \mathbb{R}$ would require us to map $1$ to a real number such as $1$ such that $x^2=-1$, which just isn't possible in the field of real numbers.
